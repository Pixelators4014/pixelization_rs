        ...
        if let Some(position_option) = APRIL_TAG_LOCATIONS.get(detection.id as usize) {
            if let Some(position) = position_option {
                ...
                let absolute_robot_pose = Isometry3::from_parts(
                    ...
                    nalgebra::geometry::Rotation3::from_euler_angles(april_tag_pose.rotation.euler_angles().0 + inverse_robot_pose.rotation.euler_angles().0, april_tag_pose.rotation.euler_angles().1 + inverse_robot_pose.rotation.euler_angles().1, april_tag_pose.rotation.euler_angles().2 + inverse_robot_pose.rotation.euler_angles().2),
                );
